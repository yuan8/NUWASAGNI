...
@section('content_header')
	...
	<h1>OUTPUT ARTIKEL TAHUN {{HP::fokus_tahun()}}</h1>

@stop

@section('content')
<div class="row">
	<form action="{{route('d.out.post.update',['id'=>$data->id])}}" method="post" enctype='multipart/form-data'id="content_posting">
		@csrf
		<div class="col-md-9">
		...
		<div class="form-group">
			<label>Thumbnail</label>
				<div class="text-center" id="thumbnail_box" style="cursor:pointer; min-height: 100px;line-height: 100px;width: 100%; border: 1px solid #222">
					@if($data->file_path)
            <img class="img-responsive" src="{{url($data->file_path)}}">
          @else
    ...
    <div class="form-group">
      ...
      <input type="date" value="{{\Carbon\Carbon::parse($data->publish_date)->format('Y-m-d')}}" class="form-control" name="publish_date">
      <small>
        * Jika Tanggal Publish tidak diisi maka atikel akan di publish saat telah disimpan 
      </small>
    </div>
...
@section('js')
  ...
  var data_content=<?php echo $data->content ?>;
